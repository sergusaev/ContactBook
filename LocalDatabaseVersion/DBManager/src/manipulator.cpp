...
//DBResult Manipulator::updateRow(const std::string& tableName, const DBTypes::DBEntry& oldRecordData, const DBTypes::DBEntry& updatedRecordData)
//{
//    DBResult result = removeRow(tableName, oldRecordData[0].toInt());
//    if(result == DBResult::FAIL) {
//        return result;
//    } else {
//        return  insertRow(tableName, updatedRecordData).first;
//    }
//}
...
std::string Manipulator::generateUpdateQuery(const std::string &tableName, size_t paramCount) const
{
    ...
    query += " WHERE rowid = ?";
    qDebug() << "In function Manipulator::generateUpdateQuery(<args>) generated query:" << QString::fromStdString(query) << "\n";
    return query;
}
